...
function restrict_subscription_plan_purchases($passed, $product_id) {
    ...
    if (implode(', ', $item_ids) == $product_id) {
        if($product_id == $monthly_plan_id){
            $has_monthly = true;
        }elseif($product_id == $yearly_plan_id){
            $has_yearly = true;
        }
    }

    if ($product_id == $free_plan_id && $has_free) {
        $_SESSION['has_free'] = true;
        $_SESSION['notice_msg'] = 'You have already used the free plan.<br>You can now upgrade to any other plan.<br>Thank you!';
    ...
    if ($product_id == $monthly_plan_id) {
        if ($has_monthly) {
            ...
            $_SESSION['notice_msg'] = 'You already have the Monthly plan.<br>You can Upgrade to Yearly plan.';
            wp_safe_redirect( site_url( '/pricing-tables/' ) );
            exit;
        }
    }
    if ($product_id == $yearly_plan_id) {
        if ($has_yearly) {
            $_SESSION['has_yearly'] = true;
            $_SESSION['notice_msg'] = 'You already have the Yearly plan.<br>You can Downgrade to Monthly plan.';
            wp_safe_redirect( site_url( '/pricing-tables/' ) );
            exit;
        }
    }
